...
En este apartado hay que tener en consideración que los datos estudiados (Abril y Mayo) se corresponden con la fase de "recuperación económica" del COVID-19. 
**España e Italia** tienen una evolución similar e indeterminada, correspondiente a la situación vivida durante la pandemia en cuanto a crecimiento de contagios. Por el otro lado vemos como **Portugal y EEUU** están teniendo una evolución claramente favorable en el tiempo, donde se ve que aunque sigue habiendo casos nuevos de COVID-19 sus economías se están recuperando. El crecimiento del índice americano se puede explicar que aunque tenga muchos más contagios, proviene de una economía muy potente. Por el otro lado Portugal, su ventaja ha sido tener un número muy bajo de contagios respecto a todos los países anteriores.
...
### Regresión lineal: Proporción a 20 días ~ densidad + %gente mayor + etc
...
Hacemos un pequeño repaso a los análisis realizados:

- Contrastes
Proporción de contagios: Encontramos evidencia suficiente para estimar que la proporción de contagios en España respecto a Italia, Portugal o Alemania es distintas.
ANOVA entre continentes: El modelo no era válido debido a que no cumplia la condición de homocedasticidad.

- Correlaciones
Proporción de test y proporción de contagiados: se observa que a mayor proporción de contagiados, mayor proporción de teses.
Variación índices bursátiles y número de casos: se obtenía una evolución indeterminada y similar para España e Italia, mientras que para EEUU y Portugal era notablemente favorable.

- Regresiones
Time Series Forecasting, ARIMA: Observabamos una crecida en Brasil, USA y Portugal, teniendo en cuenta una subida pronunciada en Portugal (aunque con muy bajas tasas de contagios), y una velocidad de aumento en Brasil mayor que en USA. Mientras tanto en Italia y España se predice una estabilización en ambas.
Proporción a 20 días ~ densidad + %gente mayor + etc: no se ha sido capaz de abstraer la estructura y el porqué de la distribución con el modelo lineal.

En definitiva, se ha visto como la afectación del COVID-19 por países y continentes ha sido muy diferente entre ellos, dependiendo de factores que han intentado ser explicados mediante indicadores sociales y demográficos. A pesar de ello no se ha encontrado una relación lineal, por ello los países más afectados tendrían que actuar en consecuencia en futuras pandemias, con mas precaución y medidas en función de sus características, ya que se desconoce con exactitud los factores que provoca  más afectación, y tampoco se puede predecir con exactitud el progreso del virus en un país.

# Agradecimientos

* Principalmente, agradecer a la asociación *Worldometers* [@worldo_cor], asociación de estadísticas mundiales en tiempo real, por tener los datos actualizados de manera tan rápida y en abierto. 
...
* El código fuente del scraping, actualización de datos y automatización mediante *Travis* se encuentra en ***[este enlace](https://github.com/AdrianArnaiz/scrap_uoc)***. Además hay archivos `readme.md` en los directorios que dan más información adiccional del proyecto.

* El dataset (conjunto de 5 csv) se sube a *Zenodo*. Cabe **destacar que debido al potencial añadido de la autoactualización con *Travis*, este dataset ha estado en continua actualización diaria de los datos hasta el 28/05/2020**, por lo tanto, Zenodo se ha actualizado con est última versión. El dataset con mayor actualización se coresponderá al que tenemos en el repositorio.
  * **DOI de Zenodo**: 10.5281/zenodo.3886437
  * **Link a Zenodo**: https://zenodo.org/record/3886437#.Xt9OmkUzaHs

A continuación, se muestra una captura del la cantidad de descargas y visitas que han tenido los datasets en la plataforma Zenodo. Queda así plasmada la actual importancia y preocupación sobre el COVID-19, así como la necesidad de encontrar información para analizar al respecto.  

```{r fig.width=3,fig.align='center',echo=FALSE}
library(png)
library(grid)
img <- readPNG("img//zenodostats.png")
grid.raster(img)
```
